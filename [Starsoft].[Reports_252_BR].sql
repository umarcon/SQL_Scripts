...
CREATE PROCEDURE [Starsoft].[Reports_252_BR]
...
-- Armazeno os lançamentos Contábeis na Conta de Clientes 1102010001
	SELECT * 
	INTO #TMP_RAZAO
	FROM [Starsoft].[fnReports_252_BR](@initial_date,@final_date)
	...
	FROM
		...
		LEFT JOIN StarWestcon.dbo.F12 F12 (NOLOCK) ON F12.F12_IUKEYP = J10.UKEY AND F12.F12_016_C = '001' -- SOMENTE NOTAS QUE GERARAM CONTAS A RECEBER
		LEFT JOIN StarWestcon.dbo.F13 F13 (NOLOCK) ON F13.F12_UKEY = F12.UKEY
	WHERE
		J10.J10_002_N = 1 
	...
	FROM
		(
			...
			FROM
				StarWestcon.dbo.J10 J10 (NOLOCK)
				INNER JOIN StarWestcon.dbo.CIA CIA (NOLOCK) ON J10.CIA_UKEY = CIA.UKEY
				INNER JOIN StarWestcon.dbo.F12 F12 (NOLOCK) ON F12.F12_IUKEYP = J10.UKEY
				INNER JOIN StarWestcon.dbo.F13 F13 (NOLOCK) ON F13.F12_UKEY = F12.UKEY
				INNER JOIN (	
								SELECT 
				...
				INNER JOIN StarWestcon.dbo.F15 F15 (NOLOCK) ON F15.F18_UKEY = F18.UKEY AND F15.F12_UKEY = F12.UKEY AND F15.F13_UKEY = F13.UKEY
			WHERE
...
-- *******************************   QUERY DE NOTAS DE DEVOLUCAO DE VENDAS - FIM ****************************************	
	...
	SELECT 
		...
		ISNULL((SELECT ROUND(SUM(F15T.F15_018_B + F15T.F15_006_B + F15T.F15_009_B + F15T.F15_010_B),2) FROM STARWESTCON.DBO.F15 F15T (NOLOCK) JOIN STARWESTCON.DBO.F18 F18T (NOLOCK) ON F15T.F18_UKEY = F18T.UKEY WHERE F15T.F13_UKEY = F13.UKEY AND F15T.F15_005_C = '002' AND F18T.F18_004_N = 1),0) AS VALOR_TOTA_JUROS,
		ISNULL((SELECT ROUND(SUM(F15T.F15_008_B),2) FROM STARWESTCON.DBO.F15 F15T (NOLOCK) JOIN STARWESTCON.DBO.F18 F18T (NOLOCK) ON F15T.F18_UKEY = F18T.UKEY WHERE F15T.F13_UKEY = F13.UKEY AND F15T.F15_005_C = '002' AND F18T.F18_004_N = 1),0) AS VALOR_TOTA_ABATIMENTO,
		ISNULL((SELECT ROUND(SUM(F15T.F15_003_B),2) FROM STARWESTCON.DBO.F15 F15T (NOLOCK) JOIN STARWESTCON.DBO.F18 F18T (NOLOCK) ON F15T.F18_UKEY = F18T.UKEY WHERE F15T.F13_UKEY = F13.UKEY AND F15T.F15_005_C = '002' AND F18T.F18_004_N = 1),0) AS VALOR_TOTAL_QUITADO,
	...
	FROM 
		...
		JOIN StarWestcon.dbo.F15 F15 (NOLOCK) ON F15.F18_UKEY = F18.UKEY AND F15.F15_005_C = '002' -- SOMENTE QUITACAO
		JOIN StarWestcon.dbo.F12 F12 (NOLOCK) ON F15.F12_UKEY = F12.UKEY
		JOIN StarWestcon.dbo.CIA CIA (NOLOCK) ON F12.CIA_UKEY = CIA.UKEY
		JOIN StarWestcon.dbo.F13 F13 (NOLOCK) ON F15.F13_UKEY = F13.UKEY
		JOIN StarWestcon.dbo.A03 A03 (NOLOCK) ON F12.A03_UKEY = A03.UKEY
	WHERE 
	...
	SELECT 
		...
		--END AS MOTIVO,
		CASE	
			WHEN SUM(TMP_QUITACAO.VALOR_QUITADO_PERIODO + TMP_QUITACAO.ABATIMENTO - TMP_QUITACAO.JUROS) <> SUM(TMP_RAZAO.VALORLANCAMENTO) THEN 'DIFERENCA ENTRE FINANCEIRO E CONTABILIDADE2'
			WHEN TMP_QUITACAO.VALOR_TOTAL_QUITADO > TMP_QUITACAO.VALOR_PARCELA - SUM(TMP_QUITACAO.VALOR_TOTA_ABATIMENTO) + SUM(TMP_QUITACAO.VALOR_TOTA_JUROS) THEN 'VALOR QUITADO MAIOR QUE O VALOR DA PARCELA'
		ELSE  
			'SEM ERRO'
		END AS MOTIVO,		
		NULL AS OBSERVACAO,
	...
	SELECT
		...
		'' AS HISTORICO,
		'' AS MOTIVO,
		'SAÍDA EM CONTA BANCARIA' AS OBSERVACAO,
		CIA_001_C AS EMPRESA,
		F18.UKEY AS DOCUMENTO_UKEY,
		F17.UKEY AS F17_UKEY
		INTO #TMP_SAIDA_BANCARIA
	FROM
		StarWestcon.dbo.F17 (NOLOCK)
		INNER JOIN StarWestcon.dbo.A01 (NOLOCK) ON F17.A01_UKEY = A01.UKEY
		INNER JOIN StarWestcon.dbo.A38 (NOLOCK) ON F17.A38_UKEY = A38.UKEY
		INNER JOIN StarWestcon.dbo.A39 (NOLOCK) ON F17.A39_UKEY = A39.UKEY
		INNER JOIN StarWestcon.dbo.F18 (NOLOCK) ON F17.F18_UKEY = F18.UKEY
		INNER JOIN StarWestcon.dbo.A21 (NOLOCK) ON F17.A21_UKEY = A21.UKEY
		INNER JOIN StarWestcon.dbo.CIA (NOLOCK) ON F18.CIA_UKEY = CIA.UKEY
	WHERE
	...
	--JOIN STARWESTCON.DBO.B06 AS B06 ON B06.B06_UKEYP = DOCUMENTO_UKEY AND B06_PAR='F18'
	...
	SELECT 
		...
		ISNULL((SELECT ROUND(SUM(F15T.F15_002_B),2) FROM STARWESTCON.DBO.F15 F15T (NOLOCK) JOIN STARWESTCON.DBO.F18 F18T (NOLOCK) ON F15T.F18_UKEY = F18T.UKEY WHERE F15T.F12_UKEY = F12.UKEY AND F15T.F15_005_C = '002' AND F18T.F18_004_N = 1 AND F18T.F18_003_D BETWEEN @initial_date AND @final_date),0) AS VALOR_QUITADO_PERIODO,
		ISNULL((SELECT ROUND(SUM(F15T.F15_002_B),2) FROM STARWESTCON.DBO.F15 F15T (NOLOCK) JOIN STARWESTCON.DBO.F18 F18T (NOLOCK) ON F15T.F18_UKEY = F18T.UKEY WHERE F15T.F12_UKEY = F12.UKEY AND F15T.F15_005_C = '002' AND F18T.F18_004_N = 1),0) AS VALOR_TOTAL_QUITADO,		
		F12.F12_002_D AS EMISSAO,
		RTRIM(A03.A03_003_C) AS CLIENTE,
		'' AS LANCAMENTO,
		0  AS VALOR_RAZAO,
		'' AS HISTORICO,
		CAST( IIF(F12.F12_IPAR='J10','CR SEM NOTA FISCAL DE ORIGEM','NAO TEM LANCAMENTO NA CONTABILIDADE') AS CHAR(254)) AS MOTIVO,
		CAST('' AS CHAR(254)) AS OBSERVACAO,
		CAST(CIA.CIA_001_C AS CHAR(254)) AS EMPRESA,
		F12.UKEY AS DOCUMENTO_UKEY,
		space(20) as DOCUMENTO_UKEY2
	FROM
		StarWestcon.dbo.F12 F12 (NOLOCK)
		JOIN StarWestcon.dbo.F13 F13 (NOLOCK) ON F13.F12_UKEY = F12.UKEY
		JOIN StarWestcon.dbo.A03 A03 (NOLOCK) ON F12.F12_UKEYP = A03.UKEY
		JOIN StarWestcon.dbo.CIA CIA (NOLOCK) ON F12.CIA_UKEY = CIA.UKEY
	WHERE
		F12.F12_016_C = '001' -- CONTAS A RECEBER
		AND F12.F12_002_D BETWEEN @initial_date AND @final_date
		AND ( ISNULL(F12.F12_IUKEYP,'') = '' or ( ISNULL(F12.F12_IUKEYP,'') <> '' AND NOT exists( SELECT J10.UKEY FROM StarWestcon.dbo.J10 J10 (NOLOCK) WHERE J10.UKEY = F12.F12_IUKEYP ) ) )
		AND NOT EXISTS (SELECT UKEY FROM StarWestcon.dbo.B06 B06 (NOLOCK) WHERE B06.B06_UKEYP = F12.UKEY)
	GROUP BY
		F12.F12_001_C,
		F12.F12_002_D,
		A03.A03_003_C,
