...
CREATE PROCEDURE [Starsoft].[Reports_252_MX]
...
-- Armazeno os lançamentos Contábeis na Conta de Clientes 1102010001
	SELECT * 
	INTO #TMP_RAZAO
	FROM [Starsoft].[fnReports_252_MX](@initial_date,@final_date)
	...
	FROM
		StarWestconMX.dbo.J10 J10 (NOLOCK)
		INNER JOIN StarWestconMX.dbo.A03 A03 (NOLOCK) ON J10.A03_UKEY = A03.UKEY
		INNER JOIN StarWestconMX.dbo.CIA CIA (NOLOCK) ON J10.CIA_UKEY = CIA.UKEY
		LEFT JOIN StarWestconMX.dbo.J06 J06 (NOLOCK) ON J06.J06_UKEYP = J10.UKEY
		LEFT JOIN StarWestconMX.dbo.F12 F12 (NOLOCK) ON F12.F12_IUKEYP = J10.UKEY AND F12.F12_016_C = '001' -- SOMENTE NOTAS QUE GERARAM CONTAS A RECEBER
		LEFT JOIN StarWestconMX.dbo.F13 F13 (NOLOCK) ON F13.F12_UKEY = F12.UKEY
	WHERE
		J10.J10_002_N = 1 
	...
	FROM
		(
			...
			FROM
				StarWestconMX.dbo.J10 J10 (NOLOCK)
				INNER JOIN StarWestconMX.dbo.CIA CIA (NOLOCK) ON J10.CIA_UKEY = CIA.UKEY
				INNER JOIN StarWestconMX.dbo.F12 F12 (NOLOCK) ON F12.F12_IUKEYP = J10.UKEY
				INNER JOIN StarWestconMX.dbo.F13 F13 (NOLOCK) ON F13.F12_UKEY = F12.UKEY
				INNER JOIN (	
								SELECT 
								...
								FROM
									StarWestconMX.dbo.J10 J10_DEV (NOLOCK) 
									INNER JOIN StarWestconMX.dbo.J11 J11_DEV (NOLOCK) ON J11_DEV.J10_UKEY = J10_DEV.UKEY 
									INNER JOIN StarWestconMX.dbo.J11 J11 (NOLOCK) ON J11_DEV.J11_UKEYP = J11.UKEY
								GROUP BY
				...
				INNER JOIN StarWestconMX.dbo.A03 A03 (NOLOCK) ON TMP_J10DEV.A03_UKEY = A03.UKEY
				LEFT JOIN StarWestconMX.dbo.J06 J06 (NOLOCK) ON J06.J06_UKEYP = TMP_J10DEV.UKEY
				INNER JOIN StarWestconMX.dbo.F18 F18 (NOLOCK) ON F18.F18_IUKEYP = J10.UKEY AND F18.F18_DUKEYP = TMP_J10DEV.UKEY
				INNER JOIN StarWestconMX.dbo.F15 F15 (NOLOCK) ON F15.F18_UKEY = F18.UKEY AND F15.F12_UKEY = F12.UKEY AND F15.F13_UKEY = F13.UKEY
			WHERE
...
-- *******************************   QUERY DE NOTAS DE DEVOLUCAO DE VENDAS - FIM ****************************************	
	...
	SELECT 
		...
		ISNULL((SELECT ROUND(SUM(F15T.F15_018_B + F15T.F15_006_B + F15T.F15_009_B + F15T.F15_010_B),2) FROM StarWestconMX.DBO.F15 F15T (NOLOCK) JOIN StarWestconMX.DBO.F18 F18T (NOLOCK) ON F15T.F18_UKEY = F18T.UKEY WHERE F15T.F13_UKEY = F13.UKEY AND F15T.F15_005_C = '002' AND F18T.F18_004_N = 1),0) AS VALOR_TOTA_JUROS,
		ISNULL((SELECT ROUND(SUM(F15T.F15_008_B),2) FROM StarWestconMX.DBO.F15 F15T (NOLOCK) JOIN StarWestconMX.DBO.F18 F18T (NOLOCK) ON F15T.F18_UKEY = F18T.UKEY WHERE F15T.F13_UKEY = F13.UKEY AND F15T.F15_005_C = '002' AND F18T.F18_004_N = 1),0) AS VALOR_TOTA_ABATIMENTO,
		ISNULL((SELECT ROUND(SUM(F15T.F15_003_B),2) FROM StarWestconMX.DBO.F15 F15T (NOLOCK) JOIN StarWestconMX.DBO.F18 F18T (NOLOCK) ON F15T.F18_UKEY = F18T.UKEY WHERE F15T.F13_UKEY = F13.UKEY AND F15T.F15_005_C = '002' AND F18T.F18_004_N = 1),0) AS VALOR_TOTAL_QUITADO,
	...
	INTO #TMP_QUITACAO
	FROM 
		StarWestconMX.dbo.F18 F18 (NOLOCK)
		JOIN StarWestconMX.dbo.F15 F15 (NOLOCK) ON F15.F18_UKEY = F18.UKEY AND F15.F15_005_C = '002' -- SOMENTE QUITACAO
		JOIN StarWestconMX.dbo.F12 F12 (NOLOCK) ON F15.F12_UKEY = F12.UKEY
		JOIN StarWestconMX.dbo.CIA CIA (NOLOCK) ON F12.CIA_UKEY = CIA.UKEY
		JOIN StarWestconMX.dbo.F13 F13 (NOLOCK) ON F15.F13_UKEY = F13.UKEY
		JOIN StarWestconMX.dbo.A03 A03 (NOLOCK) ON F12.A03_UKEY = A03.UKEY
	WHERE 
	...
	FROM
		StarWestconMX.dbo.F17 (NOLOCK)
		INNER JOIN StarWestconMX.dbo.A01 (NOLOCK) ON F17.A01_UKEY = A01.UKEY
		INNER JOIN StarWestconMX.dbo.A38 (NOLOCK) ON F17.A38_UKEY = A38.UKEY
		INNER JOIN StarWestconMX.dbo.A39 (NOLOCK) ON F17.A39_UKEY = A39.UKEY
		INNER JOIN StarWestconMX.dbo.F18 (NOLOCK) ON F17.F18_UKEY = F18.UKEY
		INNER JOIN StarWestconMX.dbo.A21 (NOLOCK) ON F17.A21_UKEY = A21.UKEY
		INNER JOIN StarWestconMX.dbo.CIA (NOLOCK) ON F18.CIA_UKEY = CIA.UKEY
	WHERE
	...
	--JOIN StarWestconMX.DBO.B06 AS B06 ON B06.B06_UKEYP = DOCUMENTO_UKEY AND B06_PAR='F18'
	...
	SELECT 
		...
		ISNULL((SELECT ROUND(SUM(F15T.F15_002_B),2) FROM StarWestconMX.DBO.F15 F15T (NOLOCK) JOIN StarWestconMX.DBO.F18 F18T (NOLOCK) ON F15T.F18_UKEY = F18T.UKEY WHERE F15T.F12_UKEY = F12.UKEY AND F15T.F15_005_C = '002' AND F18T.F18_004_N = 1 AND F18T.F18_003_D BETWEEN @initial_date AND @final_date),0) AS VALOR_QUITADO_PERIODO,
		ISNULL((SELECT ROUND(SUM(F15T.F15_002_B),2) FROM StarWestconMX.DBO.F15 F15T (NOLOCK) JOIN StarWestconMX.DBO.F18 F18T (NOLOCK) ON F15T.F18_UKEY = F18T.UKEY WHERE F15T.F12_UKEY = F12.UKEY AND F15T.F15_005_C = '002' AND F18T.F18_004_N = 1),0) AS VALOR_TOTAL_QUITADO,		
		...
		space(20) as DOCUMENTO_UKEY2
	FROM
		StarWestconMX.dbo.F12 F12 (NOLOCK)
		JOIN StarWestconMX.dbo.F13 F13 (NOLOCK) ON F13.F12_UKEY = F12.UKEY
		JOIN StarWestconMX.dbo.A03 A03 (NOLOCK) ON F12.F12_UKEYP = A03.UKEY
		JOIN StarWestconMX.dbo.CIA CIA (NOLOCK) ON F12.CIA_UKEY = CIA.UKEY
	WHERE
		F12.F12_016_C = '001' -- CONTAS A RECEBER
		AND F12.F12_002_D BETWEEN @initial_date AND @final_date
		AND ( ISNULL(F12.F12_IUKEYP,'') = '' or ( ISNULL(F12.F12_IUKEYP,'') <> '' AND NOT exists( SELECT J10.UKEY FROM StarWestconMX.dbo.J10 J10 (NOLOCK) WHERE J10.UKEY = F12.F12_IUKEYP ) ) )
		AND NOT EXISTS (SELECT UKEY FROM StarWestconMX.dbo.B06 B06 (NOLOCK) WHERE B06.B06_UKEYP = F12.UKEY)
	GROUP BY
		F12.F12_001_C,
		F12.F12_002_D,
		A03.A03_003_C,
